...
public class JdbcMigrationLauncher implements RollbackListener
{
	...
	getMigrationProcess().addMigrationTaskSource(
		new SqlScriptMigrationTaskSource());
	getMigrationProcess().addMigrationTaskSource(
		new FlatXmlDataSetTaskSource());
    }
     ...
     * 
     * @param context
     *                the database context to run the patches in
     * @param rollbackLevel
     * 		      the level the system should rollback to
     * @return the number of patches applied
     * @throws SQLException
     *                 if an unrecoverable database error occurs while working
     *                 with the patches table.
     * @throws MigrationException
     *                 if an unrecoverable error occurs during the migration
     */
	...
	try
	{
	    ...
	    try
	    {
		executedPatchCount = migrationProcess.doRollbacks(patchLevel,
			rollbackLevel, context);
	    }
	...
	{
	    // If there was any kind of error, we don't want to eat it, but we
	    // do
	...
	try
	{
	    ...
	    for (Iterator contextIter = contexts.keySet().iterator(); contextIter
		    .hasNext();)
	    {
		JdbcMigrationContext context = (JdbcMigrationContext) contextIter
			.next();
		rollbackCount = doRollbacks(context, rollbackLevel);
		log.info("Executed " + rollbackCount + " patches for context "
			+ context);
	    }
    ...
    public void migrationSuccessful(MigrationTask task, MigrationContext ctx)
	    throws MigrationException
    {
	log.debug("Task " + task.getName() + " was successful for context "
		+ ctx + " in launcher " + this);
	int patchLevel = task.getLevel().intValue();

	// update all of our controlled patch tables
	for (Iterator patchTableIter = contexts.entrySet().iterator(); patchTableIter
		.hasNext();)
	{
	    PatchInfoStore store = (PatchInfoStore) ((Map.Entry) patchTableIter
		    .next()).getValue();
    ...
    /** {@inheritDoc} */
    public void migrationFailed(MigrationTask task, MigrationContext ctx,
     ...
     *                    if there is a database connection error, or the patch
     *                    level can't be determined
     */
    public int getDatabasePatchLevel(MigrationContext ctx)
	    throws MigrationException
    {
     ...
     * @exception MigrationException
     *                    if the next patch level can't be determined
     */
     ...
     * 
     * @param context
     *                the <code>JdbcMigrationContext</code> used for the
     *                migrations
     */
    public void addContext(JdbcMigrationContext context)
    {
	PatchInfoStore patchTable = new PatchTable(context);
	log.debug("Adding context " + context + " with patch table "
		+ patchTable + " in launcher " + this);
	contexts.put(context, patchTable);
    }

    /**
     * Returns the <code>JdbcMigrationContext</code> objects used for the
     * migrations.
     * 
     ...
     *                 if an unrecoverable error occurs during the migration
     */
    protected int doMigrations(JdbcMigrationContext context)
	    throws SQLException, MigrationException
    {
	PatchInfoStore patchTable = createPatchStore(context);
     ...
     */
    private void lockPatchStore(JdbcMigrationContext context)
	    throws MigrationException
    {
	// Patch locks ensure that only one system sharing a patch store will
	// patch
	// it at the same time.
     ...
     */
    private void waitForFreeLock(JdbcMigrationContext context)
	    throws MigrationException
    {
	PatchInfoStore piStore = (PatchInfoStore) contexts.get(context);
	...
	for (int i = 0; piStore.isPatchStoreLocked(); i++)
	{
	    ...
	    {
		log.info("Reached maximum lock poll retries ("
			+ getLockPollRetries() + "), overriding patch lock");
		...
		log
			.info("  If this isn't from a long-running patch, but a stale lock, either:");
		log
			.info("    1) run MigrationTableUnlock (probably 'ant patch.unlock')");
		log
			.info("    2) set the lockPollRetries property so the lock times out");
		log
			.info("       (this is dangerous in combination with long-running patches)");
		log
			.info("    3) set the 'patch_in_progress' in the patches table to 'F'");
		...
		} catch (InterruptedException e)
		{
		    log
			    .error(
				    "Received InterruptedException while waiting for patch lock",
				    e);
		}
	    }
	}
     ...
     * 
     * @param lockPollMillis
     *                the wait time for the patch store, in milliseconds
     */
     ...
     * 
     * @param migrationProcess
     *                the MigrationProcess to use for migrations
     */
    ...
    /**
     * Return the number of times to poll the lock before overriding it. -1 is
     * infinite
     * 
    ...
    /**
     * Set the number of times to poll the lock before overriding it. -1 is
     * infinite
     * 
     * @param lockPollRetries
     *                either -1 for infinite or number of times to poll before
     *                override
     */
     ...
     * 
     * @param contexts
     *                the collection of contexts that is a map of
     *                JDBCMigrationContext -> PatchInfoStore.
     */
    ...
    /**
     * @see com.tacitknowledge.util.migration.MigrationListener#initialize(String,
     *      Properties)
     */
    ...
    public void rollbackFailed(RollbackableMigrationTask task,
	    MigrationContext context, MigrationException e)
	    throws MigrationException
    {
	log.debug("Task " + task.getName() + " failed for context " + context,
		e);

    }

    public void rollbackStarted(RollbackableMigrationTask task,
	    MigrationContext context) throws MigrationException
    {
	log.debug("Started rollback " + task.getName() + " for context "
		+ context);
    ...
    public void rollbackSuccessful(RollbackableMigrationTask task,
	    MigrationContext context) throws MigrationException
    {
	log.debug("Rollback of task " + task.getName()
		+ " was successful for context " + context + " in launcher "
		+ this);
	int patchLevel = task.getLevel().intValue();
