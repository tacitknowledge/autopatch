...
public class DistributedMigrationProcess extends MigrationProcess
{
    ...
    private static Log log = LogFactory
	    .getLog(DistributedMigrationProcess.class);
     ...
     *                The current patch level of the database
     * @param migrationsWithLaunchers
     *                a map of migration task to launcher
     * @return count of the number of patches
     */
    protected int patchDryRun(int currentPatchLevel,
	    LinkedHashMap migrationsWithLaunchers)
    {
	int taskCount = 0;

	for (Iterator i = migrationsWithLaunchers.entrySet().iterator(); i
		.hasNext();)
	{
	    Entry entry = (Entry) i.next();
	    MigrationTask task = (MigrationTask) entry.getKey();
	    JdbcMigrationLauncher launcher = (JdbcMigrationLauncher) entry
		    .getValue();
		...
		if (log.isDebugEnabled())
		{
		    ...
		    for (Iterator j = launcher.getContexts().keySet()
			    .iterator(); j.hasNext();)
		    {
			MigrationContext launcherContext = (MigrationContext) j
				.next();
			log.debug("Task will execute in context '"
				+ launcherContext + "'");
		    }
		}
		taskCount++;
     ...
     * @param rollbackLevel
     *                the level that the system should rollback to
     * @param context
     *                information and resources that are available to the
     *                migration tasks
     * @throws MigrationException
     *                 if a rollback fails
     * @return the number of <code>RollbackableMigrationTasks</code> which
	...
	if (isPatchSetRollbackable(rollbacks))
	{
	    ...
	    for (Iterator rollbackIterator = rollbacks.iterator(); rollbackIterator
		    .hasNext();)
	    {
		...
		for (Iterator j = launcher.getContexts().keySet().iterator(); j
			.hasNext();)
		{
		    MigrationContext launcherContext = (MigrationContext) j
			    .next();
	...
	for (Iterator i = migrations.iterator(); i.hasNext();)
	{
			...
			.hasNext();)
		{
		    MigrationContext launcherContext = (MigrationContext) j
			    .next();
		    PatchInfoStore patchInfoStore = (PatchInfoStore) launcher
			    .getContexts().get(launcherContext);

		    if (task.getLevel().intValue() > patchInfoStore
			    .getPatchLevel())
		    {
		...
		// of sync
		for (Iterator iter = outOfSyncContexts.iterator(); iter
			.hasNext();)
		{
		    MigrationContext launcherContext = (MigrationContext) iter
			    .next();
		    applyPatch(launcherContext, task, true);
		    patchesApplied = true;
		}
     ...
     * 
     * @throws MigrationException
     *                 if all the controlled systems are not at the current
     *                 patch level.
     */
    protected void validateControlledSystems(int currentLevel)
	    throws MigrationException
    {
	for (Iterator it = getControlledSystems().keySet().iterator(); it
		.hasNext();)
	{
	    String systemName = (String) it.next();
	    JdbcMigrationLauncher launcher = (JdbcMigrationLauncher) getControlledSystems()
		    .get(systemName);
	    for (Iterator contextIt = launcher.getContexts().keySet()
		    .iterator(); contextIt.hasNext();)
	    {
		JdbcMigrationContext ctx = (JdbcMigrationContext) contextIt
			.next();
		PatchInfoStore patchInfoStore = (PatchInfoStore) launcher
			.getContexts().get(ctx);
		...
		if (patchLevel != currentLevel)
		{
		    String message = "Database " + ctx.getDatabaseName()
			    ...
			    + ".  " + ctx.getDatabaseName()
			    + " is at patch level "
			    + Integer.toString(patchLevel)
			    + " and the System is at patch level "
     ...
     */
    public LinkedHashMap getMigrationTasksWithLaunchers()
	    throws MigrationException
    {
	LinkedHashMap tasks = new LinkedHashMap();

	// Roll through all our controlled system names
	for (Iterator controlledSystemIter = getControlledSystems().keySet()
		.iterator(); controlledSystemIter.hasNext();)
	{
	    // Get the sub launcher that runs patches for the current name
	    String controlledSystemName = (String) controlledSystemIter.next();
	    JdbcMigrationLauncher subLauncher = (JdbcMigrationLauncher) getControlledSystems()
		    .get(controlledSystemName);

	    // Get all the tasks for that sub launcher
	    List subTasks = subLauncher.getMigrationProcess()
		    .getMigrationTasks();
	    log.info("Found " + subTasks.size() + " for system "
		    + controlledSystemName);
	    for (Iterator subTaskIter = subTasks.iterator(); subTaskIter
		    .hasNext();)
	    {
		MigrationTask task = (MigrationTask) subTaskIter.next();
		if (log.isDebugEnabled())
		{
		    Iterator launchers = subLauncher.getContexts().keySet()
			    .iterator();
		    String systemName = ((JdbcMigrationContext) launchers
			    .next()).getSystemName();
		...
		.iterator(); controlledSystemIter.hasNext();)
	{
		...
		for (Iterator subTaskIter = subTasks.iterator(); subTaskIter
			.hasNext();)
		{
		    log.debug("\tFound subtask "
			    + ((MigrationTask) subTaskIter.next()).getName());
		}
	    }
     ...
     * 
     * @param controlledSystems
     *                HashMap of system name / JdbcMigrationLauncher pairs
     */
