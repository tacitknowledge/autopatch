...
public class DistributedAutoPatchRollbackTest extends MigrationListenerTestBase
{
    ...
    private static Log log = LogFactory
	    .getLog(DistributedAutoPatchRollbackTest.class);
	...
	// Make sure we load our test launcher factory, which fakes out the data
	// source context
	System
		.getProperties()
	...
	launcher = (DistributedJdbcMigrationLauncher) factory
		.createMigrationLauncher("orchestration");
	...
	coreService
		.setPatchPath("patch.core:com.tacitknowledge.util.migration.jdbc.test");
	...
	ordersService
		.setPatchPath("patch.orders:com.tacitknowledge.util.migration.tasks.rollback");
    ...
    {
	DistributedMigrationProcess process = (DistributedMigrationProcess) getLauncher()
		.getMigrationProcess();
	...
	// Make one of the sub-tasks conflict with a sub-task from another
	// launcher
	...
	List listeners = getLauncher().getMigrationProcess().getListeners();
	assertEquals(5, getLauncher().getMigrationProcess().getListeners()
		.size());
	...
	for (Iterator controlledSystemIter = controlledSystems.keySet()
		.iterator(); controlledSystemIter.hasNext();)
	{
	    String controlledSystemName = (String) controlledSystemIter.next();
	    JdbcMigrationLauncher subLauncher = (JdbcMigrationLauncher) controlledSystems
	...
	setReportedPatchLevel(process.getControlledSystems().values(),
		currentPatchlevel);
	int patches = process.doRollbacks(currentPatchlevel,
		rollbackPatchLevel, getContext());
	...
	// need to mock the patch info stores to return the expected patch
	// levels
