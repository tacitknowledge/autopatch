...
AutoPatch is open source, and is developed transparently on [Github](http://github.com/tacitknowledge/autopatch).
...
For historical reasons (circa 2004) you will find uses of the word
...
* AutoPatch should be able to globally order patches that execute
  against multiple external systems
* AutoPatch should be capable of executing post-patching maintenance
  after patching (things like recompiling pl/sql, etc)
* AutoPatch should be capable of executing the same patch across
  multiple databases with the same schema
* AutoPatch should be capable of running in a read only way, showing
...
follow the naming convention 'patchNNNN\_{name}.sql'
(e.g. 'patch0001\_create\_initial\_schema.sql'), where the numbers are
...
standard, except that it must end with a .xml suffix (patchNNNN\_{name}.xml).
Examples:
...
As of the 0.7 release of autopatch there is a new way to configure your web 
...
Create an xml file that follows the format of [DBUnit's FlatXmlDataSet format](http://dbunit.sourceforge.net/apidocs/org/dbunit/dataset/xml/FlatXmlDataSet.html).
Also, it should follow the naming convention patchNNNN\_{name}.xml.
...
AutoPatch to load. Rollback scripts are named as follows:  patch<patch level>-rollback\_{patch name}.sql.  
An example of a valid rollback file name is patch111-rollback\_testname.sql. In this case, the 111 is the level 
...
So, patch111\_testname.sql and patch111-rollback\_testname.sql are linked. Please note that the 
name of the patch does not have any role in linking a patch and its rollback. 
In fact, patch111.sql and patch111-rollback\_foo.sql would be associated.
